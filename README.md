# BaiduPan Explorer 百度网盘高速下载链接解析
BaiduPan Explorer 是一款解析百度网盘高速下载链接的浏览器插件，单文件下载速度不受限制，浏览器单线程下载可以达到1.3MB/s，使用多线程下载器可以将速度提升到带宽上限，文件夹打包下载则速度比较慢，不建议使用打包链接下载。建议使用迅雷、IDM等多线程下载器下载。

2018.04.01更新，百度网盘主页不再支持直接生成链接，链接是通过分享页生成的，频繁的操作将会导致出现验证码，账号禁用生成链接一天，请尽量减少使用次数，尽量在分享页生成链接。
